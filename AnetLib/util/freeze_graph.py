...
def freeze_latest_checkpoint(load_dir, output_file="tensorflow_model.pb", output_node_names='output,error_map'):
  model_file = tf.train.latest_checkpoint(load_dir)
  parser = get_parser()
  FLAGS, unparsed = parser.parse_known_args(['--input_meta_graph='+model_file+'.meta', '--input_checkpoint='+model_file, '--output_graph='+output_file, '--input_binary', '--output_node_names='+output_node_names])
...
if __name__ == "__main__":
  # parser = get_parser()
  workdir = '/Users/weiouyang/workspace/ANNA_PALM_v0.2.0/__model__/'
  # FLAGS, unparsed = parser.parse_known_args(['--input_meta_graph='+workdir+'model-231999.meta', '--input_checkpoint='+workdir+'model-231999', '--output_graph=A-NET-npc-tubulin-946b.pb', '--input_binary', '--output_node_names=output,error_map'])
  # app.run(main=main, argv=[sys.argv[0]] + unparsed)
  freeze_latest_checkpoint(workdir)
